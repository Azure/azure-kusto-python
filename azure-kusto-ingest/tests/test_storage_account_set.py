...
class RankedStorageAccountSetTests(unittest.TestCase):
    ...
    def test_check_rank_when_success_rate_is_different(self):
        ...
        for current_time in range(60):
            ...
            storage_account_set.add_account_result(self.ACCOUNT_2, current_time % 10 != 0)  # ~90% success
            storage_account_set.add_account_result(self.ACCOUNT_4, current_time % 2 == 0)  # ~40% success
        ...
        self.assertIn(ranked_accounts[0].get_account_name(), [self.ACCOUNT_1, self.ACCOUNT_2])
        self.assertIn(ranked_accounts[1].get_account_name(), [self.ACCOUNT_1, self.ACCOUNT_2])
        self.assertIn(ranked_accounts[2].get_account_name(), [self.ACCOUNT_3, self.ACCOUNT_4])
        self.assertIn(ranked_accounts[3].get_account_name(), [self.ACCOUNT_3, self.ACCOUNT_4])
