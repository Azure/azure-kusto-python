...
class TableReportKustoIngestionResult(IKustoIngestionResult):
    ...
    def __init__(self, map_of_source_id_to_ingestion_status_in_table):
        self.map_of_source_id_to_ingestion_status_in_table = map_of_source_id_to_ingestion_status_in_table
    
    def get_ingestion_status_by_source_id(self, source_id):
        if source_id in self.map_of_source_id_to_ingestion_status_in_table:
            ingestion_status = self._get_ingestion_status(self.map_of_source_id_to_ingestion_status_in_table[source_id])
            return ingestion_status
        else:
            raise NonexistentSourceIdException()
    
    def get_ingestion_status_collection(self):
        return six.moves.map(self._get_ingestion_status, six.itervalues(self.map_of_source_id_to_ingestion_status_in_table))
    
    def _get_ingestion_status(self, ingestion_status_in_table):
        ingestions_status_table = _ConnectionString.parse(ingestion_status_in_table["TableConnectionString"])
        ...
        entity = table_service.get_entity(table_name=ingestions_status_table.object_name, partition_key=ingestion_status_in_table["PartitionKey"], row_key=ingestion_status_in_table["RowKey"])
