...
def test_well_known_kusto_endpoints_national_clouds():
    for c in ["https://kustozszokb5yrauyq.kusto.chinacloudapi.cn,{0}".format(CHINA_CLOUD_LOGIN),
...
def test_well_known_kusto_endpoints_proxy_test():
    for c in ["https://kusto.aria.microsoft.com,{0}".format(DEFAULT_PUBLIC_LOGIN_URL),
...
def test_well_known_kusto_endpoints_proxy_negative_test():
    for clusterName in ["https://cluster.kusto.aria.microsoft.com", "https://cluster.eu.kusto.aria.microsoft.com", "https://cluster.ade.loganalytics.io", "https://cluster.ade.applicationinsights.io", "https://cluster.adx.monitor.azure.com", "https://cluster.adx.applicationinsights.azure.cn", "https://cluster.adx.monitor.azure.eaglex.ic.gov"]:
        _check_endpoint(clusterName, DEFAULT_PUBLIC_LOGIN_URL, True)


def test_well_known_kusto_endpoints_negative():
    for clusterName in ["https://localhostess", "https://127.0.0.1.a", "https://some.azurewebsites.net", "https://kusto.azurewebsites.net", "https://test.kusto.core.microsoft.scloud", "https://cluster.kusto.azuresynapse.azure.cn"]:
        _check_endpoint(clusterName, DEFAULT_PUBLIC_LOGIN_URL, True)
