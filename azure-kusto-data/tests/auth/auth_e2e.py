...
def main():
    ...
    # Test MSI auth
    if user_msi_client_id is not None or user_msi_object_id is not None:
        kcsb = KustoConnectionStringBuilder.with_aad_managed_service_identity_authentication(cluster_uri)
        faults += run_test("Connect with system MSI", lambda: ping_server(kcsb))
    ...
    if user_msi_client_id is not None:
        ...
        faults += run_test("Connect with client id MSI", lambda: ping_server(kcsb))
    ...
    if user_msi_object_id is not None:
        ...
        faults += run_test("Connect with object id MSI", lambda: ping_server(kcsb))

    # Test App Id and Key auth
    if app_key is not None and app_id is not None:
        kcsb = KustoConnectionStringBuilder.with_aad_application_key_authentication(cluster_uri, app_id, app_key, auth_id)
        faults += run_test("Connect with App Id & Key", lambda: ping_server(kcsb))
    ...
    if app_id is not None and certificate_file is not None and certificate_private_key is not None and certificate_authority is not None:
        ...
        faults += run_test("Connect with certificate", lambda: ping_server(kcsb))

        kcsb = KustoConnectionStringBuilder.with_aad_application_certificate_authentication(
            cluster_uri, app_id, certificate_private_key, certificate_thumbprint, certificate_authority, certificate_file
        )
        faults += run_test("Connect with certificate subject name and issuer", lambda: ping_server(kcsb))
    ...
    try:
        ...
        faults += run_test("Connect with token provider", lambda: ping_server(kcsb))

        kcsb = KustoConnectionStringBuilder.with_aad_user_token_authentication(cluster_uri, token)
        faults += run_test("Connect with user provided token", lambda: ping_server(kcsb))
    ...
    try:
        ...
        if az_installed.returncode == 0:
            ...
            faults += run_test("Connect with az-cli", lambda: ping_server(kcsb))
    except Exception:
        None
...
def run_test(name: str, test: callable) -> int:
    try:
