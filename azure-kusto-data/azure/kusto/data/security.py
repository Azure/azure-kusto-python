...
class _AadHelper:
    ...
    def __init__(self, kcsb: "KustoConnectionStringBuilder", is_async: bool, proxy_dict: Optional[Dict[str, str]] = None):
        self.kusto_uri = "{0.scheme}://{0.hostname}".format(urlparse(kcsb.data_source))
        self.username = None

        if kcsb.interactive_login:
            self.token_provider = InteractiveLoginTokenProvider(
                self.kusto_uri, kcsb.authority_id, kcsb.login_hint, kcsb.domain_hint, is_async=is_async, proxy_dict=proxy_dict
            )
        elif all([kcsb.aad_user_id, kcsb.password]):
            self.token_provider = UserPassTokenProvider(
                self.kusto_uri, kcsb.authority_id, kcsb.aad_user_id, kcsb.password, is_async=is_async, proxy_dict=proxy_dict
            )
        elif all([kcsb.application_client_id, kcsb.application_key]):
            self.token_provider = ApplicationKeyTokenProvider(
                self.kusto_uri, kcsb.authority_id, kcsb.application_client_id, kcsb.application_key, is_async=is_async, proxy_dict=proxy_dict
            )
            ...
            self.token_provider = AzCliTokenProvider(self.kusto_uri, is_async=is_async, proxy_dict=proxy_dict)
            ...
            self.token_provider = DeviceLoginTokenProvider(self.kusto_uri, kcsb.authority_id, is_async=is_async, proxy_dict=proxy_dict)
