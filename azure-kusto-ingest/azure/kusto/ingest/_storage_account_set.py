...
class _RankedStorageAccountSet:
    ...
    def get_ranked_shuffled_accounts(self) -> List[_RankedStorageAccount]:
        accounts_by_tier = [list[_RankedStorageAccount]() for _ in range(len(self.tiers))]
    ...
    def print_list(ranked_accounts: list[_RankedStorageAccount]):
        for account in ranked_accounts:
            print(f"{account.get_account_name()} - {account.get_rank()}")
