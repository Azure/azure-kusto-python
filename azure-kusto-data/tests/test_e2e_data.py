...
class TestE2E:
    ...
    def test_no_redirects_fail_in_cloud(self, code):
        with KustoClient(
            KustoConnectionStringBuilder.with_azure_token_credential(f"https://statusreturner.azurewebsites.net/{code}/nocloud", self.cred, credential_keep_open=True)
        ) as client:
    ...
    def test_no_redirects_fail_in_client(self, code):
        ...
        with KustoClient(
            KustoConnectionStringBuilder.with_azure_token_credential(f"https://statusreturner.azurewebsites.net/{code}/segment", self.cred, credential_keep_open=True)
        ) as client:
            with pytest.raises(KustoServiceError) as ex:
                client.execute("db", "table")
            assert ex.value.http_response.status_code == code
    ...
    async def test_no_redirects_fail_in_cloud_async(self, code):
        async with AsyncKustoClient(
            KustoConnectionStringBuilder.with_azure_token_credential(f"https://statusreturner.azurewebsites.net/{code}/nocloud", self.async_cred, credential_keep_open=True)
        ) as client:
            with pytest.raises(KustoServiceError) as ex:
    ...
    async def test_no_redirects_fail_in_client_async(self, code):
        well_known_kusto_endpoints.add_trusted_hosts([MatchRule("statusreturner.azurewebsites.net", False)], False)
        async with AsyncKustoClient(
            KustoConnectionStringBuilder.with_azure_token_credential(f"https://statusreturner.azurewebsites.net/{code}/segment", self.async_cred, credential_keep_open=True)
        ) as client:
            with pytest.raises(KustoServiceError) as ex:
                await client.execute("db", "table")
